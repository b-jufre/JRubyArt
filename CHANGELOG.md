**v1.0.8** Update recommended processing version to version 3.1 (jdk8). Exposed Constrain functionality for ArcBall, added literate perspektive method. Introduced `rgb_color` method.  Excluded `sketchPath` variable from `proxy_java_fields`, it is now private, provide an overloaded `sketchPath` method, made SKETCH_ROOT point to an absolute path.
